...
const Home = () => {
    ...
    // keep the dropdown anchored to the chip on resize / scroll while open
    useEffect(() => {
        if (!menuOpen) return
        const sync = () => {
    ...
    const logoutUser = async () => {
        setMenuOpen(false); // close immediately — no waiting on the API
        try {
            await handleLogout();
            navigate("/login");
        } catch (err) {
    ...
    return (
        <div className="hb-page">
            ...
            <header className="hb-header">
                ...
                <div className="hb-profile">
                    ...
                    >
                        ...
                        <span className="hb-profile__text" >
            ...
            {/* Backdrop + dropdown are rendered OUTSIDE the header, fixed to viewport,
                so they can never get painted under the hero/card content and always
                receive the very first tap cleanly. Position is driven purely by the
                chipRect measurement (--anchor-top / --anchor-right below), never by
                the surrounding layout, so it always lands directly under the chip. */}
            {menuOpen && (
                <button
                    type="button"
                    className="hb-profile__backdrop"
            ...
            <div
                className={`hb-profile__dropdown ${menuOpen ? 'is-open' : ''}`}
                style={{
                    position: 'fixed',
                    top: chipRect ? `${chipRect.bottom + 8}px` : '70px',
                    right: chipRect ? `${window.innerWidth - chipRect.right}px` : '16px',
                    left: 'auto',
                    bottom: 'auto',
                }}
            >
            ...
            <footer className="hb-footer">
                <div className="hb-footer__social">
                    <a href="https://www.instagram.com/1_VIDYASAGAR_SINGH" target="_blank" rel="noreferrer" className="hb-footer__icon" aria-label="Instagram">
                    ...
                    <a href="https://www.linkedin.com/in/vidya-sagar-singh-00a558342" target="_blank" rel="noreferrer" className="hb-footer__icon" aria-label="LinkedIn">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <a href="https://github.com/1VIDYASAGAR" target="_blank" rel="noreferrer" className="hb-footer__icon" aria-label="GitHub">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
//             {/* Backdrop + dropdown are rendered OUTSIDE the header, fixed to viewport,
//                 so they can never get painted under the hero/card content and always
//                 receive the very first tap cleanly. */}
//             {menuOpen && (
//                 <button
//                     type="button"
//                     className="hb-profile__backdrop"
...
//                 className={`hb-profile__dropdown ${menuOpen ? 'is-open' : ''}`}
//                 style={chipRect ? {
//                     '--anchor-top': `${chipRect.bottom + 8}px`,
//                     '--anchor-right': `${window.innerWidth - chipRect.right}px`,
//                 } : undefined}
//             >
//                 <div className="hb-profile__dropdown-inner">
//                     <div className="hb-profile__dd-head">
